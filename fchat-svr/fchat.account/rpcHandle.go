...
func (s *accountServer) Register(ctx context.Context, in *rpcPb.RegisterReq) (*rpcPb.RegisterRsp, error) {
	...
	if err != nil {
		...
		return &rpcPb.RegisterRsp{RetCode: 1, Uid: 0, Desc: "register fail. database error."}, errors.New("register fail")
	}
	...
	if err != nil {
		...
		return &rpcPb.RegisterRsp{RetCode: 2, Uid: 0, Desc: "register fail."}, nil
	}

	var Uid int32 = 0
	if LastInsertId, err := ret.LastInsertId(); nil == err {
		Uid = int32(LastInsertId)
		flog.Debug("Register account:%s,lastInsertID:%d", in.Account, LastInsertId)
	}
...
func (s *accountServer) Login(ctx context.Context, in *rpcPb.LoginReq) (*rpcPb.LoginRsp, error) {
	...
	if err != nil {
		...
		return &rpcPb.LoginRsp{RetCode: 1, Uid: 0, Desc: "login fail.Account not exist."}, nil
	}
	if Uid <= 0 {
		flog.Info("Login account:%s not exist. Uid=%d", in.Account, Uid)
		return &rpcPb.LoginRsp{RetCode: 2, Uid: 0, Desc: "login fail.Account not exist."}, nil
	}
	...
	if rowAf, er := ret.RowsAffected(); nil != er || rowAf <= 0 {
		...
		return &rpcPb.LoginRsp{RetCode: 5, Uid: 0, Desc: "login fail."}, nil
	}
